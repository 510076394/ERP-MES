  ...
  <div class="order-form-container">
    <div class="page-header">
      ...
      <div>
        ...
        <el-button 
          v-if="isEdit && orderForm.status === 'draft'" 
          type="success" 
          @click="submitOrder"
        >提交审批</el-button>
      </div>
      ...
      <el-form ref="orderForm" :model="formData" :rules="rules" label-width="120px">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            ...
            <div v-if="formData.requisition_id" style="margin-top: 10px;">
              <el-tag type="success">已关联采购申请: {{ formData.requisition_number }}</el-tag>
...
const loadSuppliers = async () => {
  try {
    ...
    if (res.data && res.data.list) {
      ...
      console.log(`成功加载${suppliers.value.length}个供应商`);
    } else if (res.list) {
      // 处理不同的返回格式
...
const searchMaterials = async () => {
  ...
  try {
    ...
    console.log(`成功加载${materialList.value.length}个物料`);
  } catch (error) {
...
const searchRequisitions = async () => {
  ...
  try {
    ...
    console.log('获取已批准的采购申请，参数:', params);
    const res = await purchaseApi.getRequisitions(params);
    if (res.data && res.data.list) {
      requisitionList.value = res.data.list;
      requisitionPagination.total = res.data.total;
    } else if (res.list) {
      // 处理不同的返回格式
      requisitionList.value = res.list;
      requisitionPagination.total = res.total || res.list.length;
    } else {
      requisitionList.value = [];
      requisitionPagination.total = 0;
      console.warn('找不到符合条件的采购申请');
    }
  } catch (error) {
...
const confirmRequisitionSelection = async () => {
  ...
  try {
    // 获取采购申请详细信息
    const res = await purchaseApi.getRequisition(selectedRequisition.value.id);
    if (res.data) {
      // 设置关联的采购申请
...
const submitForm = async () => {
  ...
  try {
    ...
    if (route.params.id) {
      // 更新
      res = await purchaseApi.updateOrder(route.params.id, formDataToSubmit);
