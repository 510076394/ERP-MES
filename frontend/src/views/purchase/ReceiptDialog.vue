...
const loadWarehouses = async () => {
  try {
    ...
    if (response.data && Array.isArray(response.data.data)) {
      // 只过滤类型为warehouse的位置
      warehouseOptions.value = response.data.data.filter(item => item.type === 'warehouse');
      console.log('成功加载仓库数据，数量:', warehouseOptions.value.length);
      console.log('仓库列表:', warehouseOptions.value);
    } else {
      console.error('仓库数据格式不正确或为空:', response.data);
...
const loadReceiptDetail = async () => {
  ...
  try {
    ...
    // 填充表单数据
    receiptForm.processing_id = data.processing_id;
    receiptForm.processing_no = data.processing_no;
    receiptForm.supplier_id = data.supplier_id;
    receiptForm.supplier_name = data.supplier_name;
    receiptForm.warehouse_id = data.warehouse_id;
    receiptForm.warehouse_name = data.warehouse_name;
    receiptForm.receipt_date = data.receipt_date;
...
const handleSubmit = async () => {
  ...
  receiptFormRef.value.validate(async (valid) => {
    ...
    // 检查表单数据完整性
    console.log('准备提交入库单数据:', JSON.stringify(receiptForm));
