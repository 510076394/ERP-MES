    ...
    let query = `
      SELECT qs.*, 
        ...
        CASE 
          ...
          WHEN qs.target_type = 'product' THEN p.name
          ELSE pr.name
        END as target_name
      ...
      LEFT JOIN products p ON qs.target_type = 'product' AND qs.target_id = p.id
      LEFT JOIN processes pr ON qs.target_type = 'process' AND qs.target_id = pr.id
      WHERE 1=1
    `;
  ...
  static async createStandard(standard) {
    ...
    try {
      ...
      const [result] = await connection.query(
        `INSERT INTO quality_standards SET ?`,
        mainData
      );
      ...
      if (items && items.length > 0) {
        for (let i = 0; i < items.length; i++) {
          await connection.query(
            `INSERT INTO quality_standard_items SET ?`,
            {
              ...items[i],
  ...
  static async updateStandard(id, data) {
    ...
    try {
      ...
      if (Object.keys(mainData).length > 0) {
        await connection.query(
          `UPDATE quality_standards SET ? WHERE id = ?`,
          [mainData, id]
        );
      }
      ...
      if (items && items.length > 0) {
        await connection.query(
          `DELETE FROM quality_standard_items WHERE standard_id = ?`,
          [id]
        );
